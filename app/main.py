...
from minio.select import SelectRequest, CSVInputSerialization, CSVOutputSerialization, JSONOutputSerialization
...
def post_data():
    ...
    for file in csv_files:
        ...
        with open(file, 'r') as f:
            ...
            img_path = f"{user_id}.png" if os.path.exists(f"app/02-src-data/{user_id}.png") else ""
            rows.append([user_id] + [field.strip() for field in next(csvreader)] + [img_path])
    ...
    return {"header": header, "rows": rows, "buckets": buckets}
